...
class ViewController: FDDBaseTableViewController {
    ...
    func disposeDataSources () {
        let randomSources = ["Swift is now open source!",
                             "We are excited by this new chapter in the story of Swift. After Apple unveiled the Swift programming language, it quickly became one of the fastest growing languages in history. Swift makes it easy to write software that is incredibly fast and safe by design.",
                             "Learning Swift has been a great introduction to modern programming concepts and best practices. And because it is now open, their Swift skills will be able to be applied to an even broader range of platforms, from mobile devices to the desktop to the cloud.",
    ...
    func disposeTableViewConverter() {
        ...
        self.tableViewConverter?.registerTableViewMethod(selector: #selector(UITableViewDelegate.tableView(_:didSelectRowAt:)), handleParams: { [weak self]  (_ params: Array) -> AnyObject? in
        ...
        self.tableViewConverter?.registerTableViewMethod(selector: #selector(UITableViewDataSource.tableView(_:cellForRowAt:)), handleParams: { [weak self] (_ params : Array) -> AnyObject? in
